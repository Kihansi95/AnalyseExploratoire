...
toto = separate(olympic_dataset, Games, c("Year_Games", "Season"), sep = " ")  # J'ajoute une nouvelle colonne pour indiquer les saisons
...
## Contexte
Les **Jeux Olympiques** (JO) sont une compétition que nous avons hérité de la Grèce Antique. On lit souvent que seuls les citoyens les plus riches pouvaient y participer. La trève olympique permettait à la civilisation d'oublier, le temps de 12 travaux, les guerres et troubles de l'époque.
Après quelques maigres tentatives, c'est seulement depuis 1896 que les Jeux Olympiques modernes voient le jour, à Athènes.

Dans le cadre du projet d'**Analyse Exploratoire** à l'**INSA Toulouse**, avec l'accompagnement de professeur **Gilles Tredan**, nous avons récupéré sur le site Kaggle un dataset sur ces JO moderne [@Olympic_dataset]. Ce dernier recense des informations sur tous les athlètes ayant participés aux JO, les médailles qu'ils ont gagnés, leur équipe, l'année de participation, etc.

Sauf précision contraire, les données sont exploitées ici dans leur totalité, depuis la création des Jeux Olympiques modernes (1896). Suivant l'analyse, les données incomplètes ont été traitées différemment. Parfois elles ont été mise à 0. D'autre fois, on a préférer les ignorer.

## Problématique

Au delà du caractère spectaculaire et sportif, les Jeux Olympics sont un sujet intéressant pour les scientifiques : Le dernier sciècle a prédit que l'humain atteindra bientôt ses limites physiques, tandis que l'on voit toujours aujourd'hui des sportifs continuer d'établir de nouveaux records.

Avec plusieurs centaines d'années d'existance des JO, des facteurs optimisant les performances des athlètes ont été identifié. L'historique des JO pourraient surement fournir une indication à la question: Quels sont les facteurs qui influent/optimisent la performance physique des athlètes?

On peut aussi se demander quelle est la place de la femme, encore trop souvent controversé dans notre société actuelle.


# 1. Premièr axe: Les caractéristiques physiques des athlètes influencent leur performance
...
Cette formule n'est pas adaptée lorsque l'on a un seul participant. Car s'il gagne une médaille, la probabilité sera de 100%. On peut voir dans la figure à gauche un pique à l'âge de 10: le seul participant à cet âge **Dimitrios Loundras** en 1896 qui a gagné 1 médaille.
...
perf <- olympic_dataset %>%
    ...
    xlab("Age")+
    ylab("Performance") +
    ggtitle("Chance de gagner médaille")+
    labs(subtitle="Plus haute la valeur,\n plus probable qu'on gagne.")
...
perf_clean <- olympic_dataset %>%
    ...
    ylab("Performance") +
    ggtitle("Chance de gagner médaille")+
    labs(subtitle="Seulement les données à plus de\n 500 observations")

grid.arrange(perf, perf_clean, ncol = 2)
```

Dans le diagramme à droit, en filtrant les catégories dont le nombre d'observation n'est pas "suffisant", on voit que l'âge idéale pour gagner de médaille est à la fin de 20 devant 30 ans.
...
ggplot(medals_year,aes(Year, Medals, group=Sex)) +
    geom_col(position="dodge",aes(fill=Sex)) +
    labs(x="Années", y="Total Medailles", title = "Evolution du nombre de médailles gagnées par sexe")


# je ne sais pas ce que tu veux faire en attibuant Year = Year - 2 pour les année 1994 - 2014.
# Tu peux faire mieux: medals_year$Year[medals_year$Season=="Winter"] <- medals_year$Year - 2
# Voir le code en bas si c'est ça tu veux faire ...

#proportion femme-homme parmis les athlètes
homme_femme <- ddply(olympic_dataset, .(as.numeric(as.character(Year))), function(x){ data.frame(Proportion = sum(x$Sex=="F")/(sum(x$Sex=="F")+sum(x$Sex=="M")))})
```
TODO: Anais check out dans le code

```{r women_man}
olympic_dataset %>%
    count(Year, Sex, Season, Medal) %>%  # -----
    subset(Medal != "None") %>%          #   | 
    spread(Medal, n, fill=0) %>%         #   |    ça remplace ddply, tu peux comparer le résultat des 2
    mutate(                              #   |
        Medals = Bronze + Gold + Silver, # ----- 
        Year = replace(Year, Season == "Winter", Year - 2) # ça remplace treat case of winter and summer
    ) %>%
    ggplot(aes(Year, Medals, group=Sex)) +
    geom_col(position="dodge",aes(fill=Sex)) +
    labs(x="Années", y="Total Medailles", title = "Evolution du nombre de médailles gagnées par sexe")
```
...
## 2.2 Les femmes et l'âge de la sagesse sportive
...
# Conclusion
Axes d'ouverture : Richesse des pays
